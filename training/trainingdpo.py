...
train_files = sorted(glob.glob(os.path.join(base_dir, "*train.json*")))

# Process each dataset
for train_path in train_files:
    base_name = os.path.basename(train_path).split("train.")[0]

    eval_path = os.path.join(base_dir, f"{base_name}test.json")

    # Check for .jsonl if .json doesn't exist
    if not os.path.exists(eval_path):
        eval_path = os.path.join(base_dir, f"{base_name}test.jsonl")
